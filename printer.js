...
module.exports.page = async function(pid, req, res) {
    ...
    switch (pid) {
        ...
        case 1:
            if (req.query.t == null || req.query.t == 0) genericVariable = await global.Database.query(`SELECT user_id, ranked_score, pp_raw FROM users_modes_info WHERE mode_id = ${req.query.m == null ? 0 : req.query.m} ORDER BY pp_raw DESC LIMIT 50`);
            else genericVariable = await global.Database.query(`SELECT user_id, ranked_score, pp_raw FROM users_modes_info WHERE mode_id = ${req.query.m == null ? 0 : req.query.m} ORDER BY ranked_score DESC LIMIT 50`);
            ...
            for (let i = 0; i < genericVariable.length; i++) {
                ...
                if (genericVariable[i].ranked_score > 0) {
                    genericVariable2 = await global.Database.query(`SELECT username, country FROM users_info WHERE id = ${genericVariable[i].user_id} LIMIT 1`);
            ...
            try {
                if (req.query.u != null) {
                    ...
                    if (genericVariable.toString() == "NaN") {
                        ...
                        pageUser = await global.Database.query(`SELECT * FROM users_info WHERE username = "${req.query.u}" LIMIT 1`);
                    ...
                    else pageUser = await global.Database.query(`SELECT * FROM users_info WHERE id = ${genericVariable} LIMIT 1`);
                    ...
                    genericVariable = await global.Database.query(`SELECT * FROM users_modes_info WHERE mode_id = ${gameMode} AND user_id = ${pageUser.id} LIMIT 1`);
                    genericVariable1 = await global.Database.query(`SELECT max_combo FROM scores WHERE userid = ${pageUser.id} ORDER BY max_combo DESC LIMIT 1 `);
                    webPrefs = await global.Database.query(`SELECT * FROM web_prefs WHERE id = ${pageUser.id} LIMIT 1`);
                ...
                return `There was an error loading the userpage for this user: ${e}`;
            }
        ...
        case 105:
            webPrefs = await global.Database.query(`SELECT * FROM web_prefs WHERE id = ${req.user.id} LIMIT 1`);

            return `
                <div class="settingspage">
                    <div class="userbox" style="height:360px">
