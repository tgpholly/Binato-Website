...
module.exports = async function(loginInfo, req, res) {
    ...
    await global.Database.query(`INSERT INTO users_info (id, username, username_safe, password, email, country, reg_date, last_login_date, last_played_mode, online_now, tags, supporter, web_session, verification_needed, password_change_required, has_old_password) VALUES (NULL, '${loginInfo.u}', '${loginInfo.u.toLowerCase().split(" ").join("_")}', '${aes256.encrypt(config.database.databaseKey, md5(loginInfo.ha))}', '${loginInfo.em}', '${userCountry}', CURRENT_TIME(), CURRENT_TIME(), '0', '0', '4', '0', '${sessionToken}', '1', '0', '0');`);
